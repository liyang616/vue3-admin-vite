...
import { useDark, useGlobal, debounce, isNumber, useWatermark } from '@pureadmin/utils'
...
const settings = reactive({
  watermarkVal: $storage.configure.watermark,
...
/** 水印模式设置 */
const watermarkChange = (value): void => {
  storageConfigureChange('watermark', value)
  if (settings.watermarkVal) {
    watermarkSet()
  } else {
    clear()
  }
...
onBeforeMount(() => {
  ...
  nextTick(() => {
    ...
    settings.watermarkVal && watermarkSet()
...
<template>
  <LayPanel>
    <div class="p-5">
      ...
      <ul class="setting">
        <li>
          <span class="dark:text-white">{{ t('panel.pureWatermarkMode') }}</span>
          <el-switch
            v-model="settings.watermarkVal"
            inline-prompt
